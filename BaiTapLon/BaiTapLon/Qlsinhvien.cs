...
namespace BaiTapLon
{
    public partial class Qlsinhvien : Form
    {
        ...
        public void loadData()
        {
            ...
            //Kết Quả
            string sql5 = "select * from KETQUA";
        ...
        //Load lại bảng Lớp
        private void btLamMoiLop_Click(object sender, EventArgs e)
        {
        ...
        private void btSuaLop_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                string sql = String.Format(@"Update LOP set MaLop = @MaLop, TenLop = @TenLop, MaKhoa = @MaKhoa where MaLop = '{0}'", tbmasv.Text);
                string[] name = { "@MaLop" ,"@TenLop","@MaKhoa"};
                bool gt = rdNam.Checked == true ? true : false;
                object[] value = { tbmasv.Text, tbhoten.Text, dtpkdate.Value, gt, tbnienkhoa.Text, cbbmalop.SelectedValue };
                KetNoi.moketnoi();
        ...
        private void XoaMon_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                string sql = string.Format("Delete from MONHOC where MaMH = '{0}'", tbmamh.Text);
                string[] name = { };
                object[] value = { };
                KetNoi.moketnoi();
                KetNoi.updateData(sql, value, name, 0);
        ...
        private void datashow5_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtChuyenCan.Text = datashow5.Rows[i].Cells[3].Value.ToString();
            txtDieuKien.Text = datashow5.Rows[i].Cells[4].Value.ToString();
            txtCuoiKi.Text = datashow5.Rows[i].Cells[5].Value.ToString();
            txtKiHoc.Text = datashow5.Rows[i].Cells[6].Value.ToString();
        }
        //Thêm thông tin vào bảng Kết Quả
        private void btthem4_Click(object sender, EventArgs e)
        {
            string sql = "Insert into KETQUA VALUES(@MaMH, @Masv, @TenMH, @ChuyenCan,@DieuKien,@CuoiKi,@HocKi)";
            string[] name = { "@MaMH", "@Masv", "@TenMH", "@ChuyenCan" , "@DieuKien", "@CuoiKi", "@HocKi" };
            object[] value = { cbbmamonhoc.SelectedValue, cbbmasv.SelectedValue, cbbtenmh.SelectedValue, txtChuyenCan.Text ,txtDieuKien.Text,txtCuoiKi.Text,txtKiHoc.Text};
            KetNoi.moketnoi();
        ...
        private void btSuaKQ_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                string sql = String.Format(@"Update KETQUA set MaMH = @MaMH, Masv = @Masv, TenMH = @TenMH, ChuyenCan = @ChuyenCan, DieuKien = @DieuKien, CuoiKi = @CuoiKi, KiHoc = @KiHoc where Masv = '{0}' and MaMH ='{1}' ",cbbmasv.SelectedValue, cbbmamonhoc.SelectedValue);
                string[] name = { "@MaMH", "@Masv", "@TenMH", "@ChuyenCan", "@DieuKien", "@CuoiKi" , "@KiHoc" };
                object[] value = { cbbmamonhoc.SelectedValue, cbbmasv.SelectedValue, cbbtenmh.SelectedValue, txtChuyenCan.Text, txtDieuKien.Text,txtCuoiKi.Text, txtKiHoc.Text};
                KetNoi.moketnoi();
                KetNoi.updateData(sql, value, name, 7);
                loadData();
        ...
        {
            string sql = string.Format("select * from KETQUA where Masv = '{0}'", cbbmasv.SelectedValue);
            KetNoi.moketnoi();
            datashow4.DataSource = KetNoi.getData(sql);
            KetNoi.dongketnoi();
        }
        ...
        private void btXoaKQ_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                string sql = string.Format("Delete from KETQUA where Masv = '{0}'", cbbmasv.SelectedItem);
                string[] name = { };
                object[] value = { };
                KetNoi.moketnoi();
                KetNoi.updateData(sql, value, name, 0);
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                string sql = string.Format("Delete from USER where Masv = '{0}'", cbbtaikhoan.SelectedItem);
                string[] name = { };
                object[] value = { };
                KetNoi.moketnoi();
                KetNoi.updateData(sql, value, name, 0);
        ...
        {
            string sql = string.Format("select * from USER where Masv = '{0}'", cbbtaikhoan.SelectedItem);
            KetNoi.moketnoi();
            datashow4.DataSource = KetNoi.getData(sql);
            KetNoi.dongketnoi();
        }
